...
<center>![Flappy Birb](./_03-tp3/flappyBirb.png)</center>

Le [projet Next.js est fourni](https://github.com/departement-info-cem/4W6-WebServices/releases/latest/download/code-tp3.zip) pour ce TP. Sa structure doit être respectée et complétée.
...
<center>![Résumé des requêtes à implémenter](./_03-tp3/swagger.png)</center>
...
<center>![Flappy Birb](./_03-tp3/user.png)</center>
...
<center>![Jouer à Flappy Birb](./_03-tp3/play.png)</center>
...
<center>![Affichage des scores](./_03-tp3/scores.png)</center>
...
## 👽 Exceptions pour l'usage de l'IA

La génération de code par IA sera permise pour deux **cas précis** dans ce devoir, **à condition d'indiquer que l'IA a été utilisée**.

:::warning

Si le code généré par IA est maladroit ou loin d'être optimal, vous pouvez perdre des points. L'IA est un outil pour bonifier la productivité, mais il faut être capable de valider le code généré !

:::

### 📦 Gestion des états après une requête HTTP

Le code permettant de mettre à jour des **états** après une **requête HTTP** peut être généré par IA. Voici un exemple :

```tsx showLineNumbers
async myFunction(id : number){

    const x = await axios.delete("... requête ...");
    console.log(x.data);

    // 👽 Début du code généré par IA
    setState(state.filter(s => s.id != id));
    // 👽 Fin du code généré par IA

}
```

Cela devrait vous êtes utile pour la requête de type `PUT` du devoir, par exemple. Vous aurez droit à cette exception dans le TP4 également.

### 🔗 LINQ

Les opérations exploitant des méthodes LINQ dans l'application `ASP.NET Core` peuvent être générées par l'IA. (`.Where()`, `.OrderBy()`, `.Select()`, etc.)

Attention, cela ne concerne pas un grand nombre de lignes de code : surtout quelques manipulations liées à des requêtes `GET` sur le `DbContext`.