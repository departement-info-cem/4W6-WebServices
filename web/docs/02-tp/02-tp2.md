...
Il **imite l'API de Spotify** : les requêtes sont les mêmes, seule l'**adresse** change.

:::

## 📥 Remise
- 📍 Où : sur **Git** uniquement.
  - Si votre enseignant est **Pierre-Olivier** : Vous allez recevoir une invitation à un repo GitHub.
  - Si votre enseignant est **Philippe** : Vous allez recevoir une invitation à un repo GitHub.
  - Si vous n'avez pas reçu d'invitation ou que ne savez pas comment accéder à votre TP, venez voir votre enseignant.
* ⏰ Quand : 
  - Si votre enseignant est **Pierre-Olivier** : **9 octobre 23h59**. 
  - Si votre enseignant est **Philippe** : **2 octobre 23h59**. 
  - Vous avez 4 cours (12h en classe !) consacrés au TP.
* ⛔ Remise partielle : 
  - Si votre enseignant est **Pierre-Olivier** : **29 septembre 10h59** (ou avant), **en classe**. 
  - Si votre enseignant est **Philippe** : **25 septembre 10h59** (ou avant), **en classe**. 
  - [Explications ici](/tp/tp2#-remise-partielle)
...
Header Spot-Moi avec un sélecteur de langue (français/anglais)

Un barre d'ajout d'artiste à la liste de l'utilisteur. (input + bouton «Rechercher»)

Une liste de «Vos artistes», dont chaque item contien le nom de l'artiste, sa photo, un bouton «Concerts» et un bouton «Albums».
</NonVoyant>
...
Albums de «Nom de l'artise»

Une liste des albums dont chaque item contien le nom de l'album, l'image de la pochette d'album et un bouton «Chansons».
</NonVoyant>
...
Chansons de «Nom de l'album»

Une liste des chansons dont chaque item contien le nom de la chanson et un bouton «Écouter».
</NonVoyant>
...
<NonVoyant>
Exemple HTML sous la liste des chansons:
...
Concerts de «Nom de l'artiste»

Une carte Google avec tous les concerts afficher des marqueurs.

Une liste des concerts où chaque item contient: La date et l'heure, le pays, la ville.
</NonVoyant>
...
:::tip
En date d'aujourd'hui, Drake et Nana Mouskouri n'ont pas de concerts prévus. Hans Zimmer en a quelques uns qui s'en viennent par contre!
:::
...
En **classe**, il faut montrer à l'enseignant(e) que vous avez terminé la **page d'accueil**. (Artistes) Ceci n'inclut pas l'internationalisation. Si vous ne réalisez pas cette démonstration, **vous perdez 25%**.
...
N'oubliez pas que **toutes les requêtes Spotify** (Connexion, artiste, albums et chansons) sont déjà codées pour vous et sont disponibles dans le [Cours 4.1](/notes/rencontre4.1#-exemples-de-requêtes-à-spotify-pour-le-tp2). Ce sont exactement les mêmes requêtes avec le **serveur maison** : seule l'adresse change. ([Cours 5.1](/notes/rencontre5.1)) Seule la requête **BandsInTown** sera codée par vous-même.
...
1. 📦 Créer tous les composants dont vous aurez besoin. (Cours 3.1)
...
2. ⚱ Préparer des classes (`/app/_types`) pour encapsuler les données. (Cours 1.1)
...
3. ✅ Rendre vos composants fonctionnels, un à la fois. (Cours 3.1 à 4.2)
...
4. 👅 Internationaliser. (Cours 4.1)
    * Si tout fonctionne et que vous êtes satisfait(e) de votre application, c’est l’heure de tout traduire !
    * Les seuls nouveaux défis que vous devriez rencontrer en comparaison avec le labo 4.1 est la nécessité de diviser le layout et la traduction de dates. Ces deux défis sont abordés dans les notes de cours ! 
