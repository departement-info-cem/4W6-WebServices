...
import clientServeur from './_01-rencontre1.1/animation-client-serveur'
import spa from './_01-rencontre1.1/animation-spa'
import { DataFlowPlayer } from '@dataflow-animator/react';
import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';
import JavaScriptConsole from '@site/src/components/JavaScriptConsole';
import exempleSimple from '!!raw-loader!./_01-rencontre1.1/exemple-simple.js';
import multiplyByTwoJavaScript from '!!raw-loader!./_01-rencontre1.1/multiply-by-two.js';
import multiplyByTwoTypeScript from '!!raw-loader!./_01-rencontre1.1/multiply-by-two.ts';
...
<DataFlowPlayer spec={clientServeur} theme="auto" />
<NonVoyant>
  L'animation explique les interactions du serveur web sous un structure MCV .NET.
  L'utilisateur ouvre la page via le navigateur, la requête HTTP est envoyée au serveur.
  Le serveur recoit un GET /users.
  Le serveur interroge la base de données par la requète SQL SELECT * FROM users.
  La base de données retourne deux lignes contenant les information d'Alice et Bob.
  Le serveur transforme ces lignes en document html bien formaté.
  Le navigatgeur affiche la page avec Alice et Bob.
</NonVoyant>
...
<DataFlowPlayer spec={spa} theme="auto" />
<NonVoyant>
  L'animation ici est plus complexte. Elle vient introduire un deuxième serveur. Ce nouveau serveur s'occupe principalement de fournir tous ce qui relève de l'interface.
  Tandis que le serveur ASP.NET s'occupe de tout ce qui concerne les données.
</NonVoyant>
...
<center>![Liste de différents framework backend: Spring pour Java, Ruby on Rails, .Net Core en C#, Laravel en PHP et django en Python](./_01-rencontre1.1/back_end.png)</center>
...
<center>![Liste Frameworks Web front end: Angular, React et Vue](./_01-rencontre1.1/front_end.png)</center>
...
Contrairement à Angular et Vue, React n'est pas vraiment un **framework client**, mais plutôt une **librairie**. (Avec moins de fonctionnalités qu'Angular et Vue) Des frameworks comme **Next.js**, **Gatsby**, **Vite.js**, ce site web!, etc sont construits autour de la librairie React. De notre côté, nous utiliserons **Next.js** car c'est, en date d'aujourd'hui, le framework client le plus populaire™. 💃🕺📢
...
#### NPM
<center>![](./_01-rencontre1.1/npm.png)</center>
...
<center>![](./_01-rencontre1.1/powershell.png)</center>
...et tapez simplement la commande `npm`, qui devrait vous proposer de l'aide pour taper d'autres commandes npm.

:::info
Il se peut que votre système soit plus restrictif et vous donne des erreurs lorsque vous essayez d'exécuter **npm**. Le niveau de restriction de PowerShell ne permet
pas l'exécution de scripts par défaut. Vous devez modifier la politique de sécurité de PowerShell. Vous avez deux options:

**Solution 1 : Modifier la politique pour la session actuelle (Recommandé)**

Cette méthode est la plus sûre, car elle active les scripts uniquement dans la fenêtre PowerShell ouverte. La sécurité d'origine sera rétablie dès que vous fermerez
 cette fenêtre.
1) Laissez votre fenêtre PowerShell ouverte.
2) Copiez et exécutez la commande suivante :
```powershell title="PowerShell" showLineNumbers
Set-ExecutionPolicy -ExecutionPolicy Bypass -Scope Process
```
3) Exécutez votre commande npm.

**Solution 2 : Modifier la politique de façon permanente**

Cette méthode modifie la configuration de votre ordinateur pour que PowerShell autorise toujours l'exécution de vos scripts locaux:
1) Fermez PowerShell.
2) Ouvrez un nouveau PowerShell en mode Administrateur (clique droit sur l'icône > **Exécuter en tant qu'administrateur**).
3) Copiez et exécutez la commande suivante :
```powershell title="PowerShell" showLineNumbers
Set-ExecutionPolicy -ExecutionPolicy RemoteSigned -Scope CurrentUser
```
4) Appuyez sur **O** (Oui) ou **A** (Oui pour tout) si le système demande une confirmation.
5) Vous pouvez maintenant fermer cette fenêtre et utiliser npm normalement dans vos terminaux habituels.
:::

### 🧪 Introduction à TypeScript
<center>![](./_01-rencontre1.1/typescript.png)</center>
...
<JavaScriptConsole code={exempleSimple} />
...
<JavaScriptConsole
  files={{
    "/index.js": multiplyByTwoJavaScript,
    "/index.ts": multiplyByTwoTypeScript,
  }}
/>
...
<center>
    ![La fonction multiplyByTwo("sus") retourne le symbole NaN.](./_01-rencontre1.1/oops.png)  
...
<center>![](./_01-rencontre1.1/vscode.png)</center>

Nous l'utiliserons pour travailler sur nos projets **React**. (Semaines 1 à 15)

#### Visual Studio

<center>![](./_01-rencontre1.1/vs.png)</center>
...
<center>![Fichiers de départ d'un projet Next.js](./_01-rencontre1.1/react_files.png)</center>
<NonVoyant>
  Il devrait y avoir quatre répertoires (.next, app, node_modules et public) et 
  neuf fichiers (.gitignore, eslint.config.mjs, next.config.ts, next-env.d.ts, package.json, package-lock.json, postcss.config.mjs, README.md et tsconfig.json)
</NonVoyant>
...
<center>![](./_01-rencontre1.1/node_modules_react.png)</center>
...
<center>![Commande npm install dans powershell](./_01-rencontre1.1/npm_install.png)</center>
...
<center>![Clic droit(shift + F10) sur le dossier et choisir Open with Code](./_01-rencontre1.1/open2.png)</center>

<center>... ou encore ...  </center>
  
<center>![Clic à l'intérieur du dossier et choisir Open with Code](./_01-rencontre1.1/open3.png)</center>

Pour exécuter le projet, ouvrez un terminal (Onglet `Terminal` -> `Nouveau terminal`) puis tapez la commande `npm run dev`.

<center>![Commande npm run dev](./_01-rencontre1.1/run_dev.png)</center>
<NonVoyant>Il est possible d'utiliser un terminal powershell en dehors de Visual Studio Code.</NonVoyant>
...
<center>![Page d'accueil par défaut http://localhost:3000](./_01-rencontre1.1/default_index_next.png)</center>
...
<center>![Liste des fichiers de départ Next.js dans l'explorateur Visual Studio Code](./_01-rencontre1.1/files_next.png)</center>
...
<center>![Exemple de page.tsx qui affiche Salut](./_01-rencontre1.1/page.png)</center>
...
<center>![Exemple du menu contextuel "Quick Fix" sur une méthode manquant un import from "react"](./_01-rencontre1.1/ctrlPoint.png)</center>
...
<center>![Résultat html = "Salut Simone, vous avez 37 ans."](./_01-rencontre1.1/displayVariableReact.png)</center>
...
<center>![Résultat html : "Voici une valeur : 67"](./_01-rencontre1.1/displayFunctionReact.png)</center>
...
<center>![Le fichier npc.ts qui se retrouve sous le répertoire _types](./_01-rencontre1.1/typesFolder.png)</center>
...
<center>![Quick Fix: "Add import from "./_types/npc"](./_01-rencontre1.1/importClass.png)</center>

Utilisez le **constructeur** de votre classe pour instancier un nouvel objet :

<center>![Suggestion d'auto-complétion: "Npc(name: string, dialogue: string, age: number): Npc"](./_01-rencontre1.1/constructorCallReact.png)</center>
...
<center>![Résultat html = "Khajiit est un NPC de 176 an(s) dont le dialogue est «Khajiit has wares... if you have coin.»"](./_01-rencontre1.1/displayClass.png)</center>
...
<center>![Résultat html : "J'adore les couleurs suivantes ! Youpi ! -indigo -cramoisi -ocre"](./_01-rencontre1.1/displayArrayReact.png)</center>
...
Commencez par initialiser votre repository sur GitHub. (Pour pouvoir le cloner dans **Fork** 🔱 ensuite. Vous pouvez aussi utiliser **GitHub Desktop**.)

⛔ Assurez-vous qu'il soit privé !

<center>![S'assurer que l'option Choose visibility soit à Private. ](./_01-rencontre1.1/git1.png)</center>

Clonez le repository sur votre ordinateur à l'aide de son URL :

<center>![Cloner le repository avec Fork](./_01-rencontre1.1/git2.png)</center>
...
<center>![Contenu du projet Next.js avec .gitignore](./_01-rencontre1.1/react_files_git.png)</center>
...
<center>![Commit dans le repository](./_01-rencontre1.1/commit_react.png)</center>
