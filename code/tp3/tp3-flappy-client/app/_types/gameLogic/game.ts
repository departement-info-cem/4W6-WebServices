...
export class Game {
    ...
    stop(unmount: boolean) {
        ...
        if (!unmount) {
            ...
            reaction.setAttribute("src", "/images/" + (this.score > 100 ? "very_happy" : (this.score > 50 ? "happy" : (this.score > 10 ? "sad" : "angry"))) + ".png");
            this.score = 0;
            this.time = 0;
    ...
    update() {
        ...
        for (let i = this.barriers.length - 1; i >= 0; i--) {
            ...
            if (this.barriers[i].intercepts(birbHitbox)) this.stop(false);
